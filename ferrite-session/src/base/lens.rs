...
use crate::{
  base::{channel::ReceiverOnce, context::Context, protocol::Protocol},
  functional::nat::{S, Z},
};
...
pub trait GenericLensCont<N, C, D, A, B, K>
{
  ...
  where
    ...
    A : Slot,
    B : Slot,
    N : ContextLens<C, A, B, Deleted = D>;
}

pub trait GenericLens<C, D, A>: Sized + Send + 'static
{
  fn with_lens<B, K>(cont : impl GenericLensCont<Self, C, D, A, B, K>) -> K
  where
    B : Slot;
}
...
impl<C, A> GenericLens<(A, C), C, A> for Z
where
  A : Slot,
  C : Context,
{
  fn with_lens<B, K>(cont : impl GenericLensCont<Z, (A, C), C, A, B, K>) -> K
  where
    B : Slot,
  {
...
struct SuccLensCont<Cont, N, C, D, A, B1, B2, K>
where
  A : Slot,
  Cont : GenericLensCont<S<N>, (A, C), (A, D), B1, B2, K>,
{
  cont : Cont,
  phantom : PhantomData<(N, C, D, A, B1, B2, K)>,
}

impl<Cont, N, C, D, A, B1, B2, K> GenericLensCont<N, C, D, B1, B2, K>
  for SuccLensCont<Cont, N, C, D, A, B1, B2, K>
where
  A : Slot,
  Cont : GenericLensCont<S<N>, (A, C), (A, D), B1, B2, K>,
{
  ...
  where
    ...
    B1 : Slot,
    B2 : Slot,
    N : ContextLens<C, B1, B2, Deleted = D>,
  {
...
impl<N, C, D, A, B1> GenericLens<(A, C), (A, D), B1> for S<N>
where
  C : Context,
  D : Context,
  A : Slot,
...
{
  fn with_lens<B2, K>(
    cont1 : impl GenericLensCont<S<N>, (A, C), (A, D), B1, B2, K>
  ) -> K
  where
    B2 : Slot,
  {

    let cont2 = SuccLensCont::<_, N, C, D, A, B1, B2, K> {
      cont : cont1,
      phantom : PhantomData,
    };
